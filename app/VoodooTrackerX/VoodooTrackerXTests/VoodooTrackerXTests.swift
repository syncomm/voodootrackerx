...
private func makeRampPlaybackSample(
    frameCount: Int = 600,
    instrumentIndex: Int = 1,
    sampleIndex: Int = 0,
    volume: Float = 1,
...
private func XCTAssertPCMEqual(
    _ actual: [Float],
    _ expected: [Float],
...
private func renderRuntimePCM(_ core: RuntimeCMixerRenderCore, frames: Int) -> [Float] {
...
final class VoodooTrackerXTests: XCTestCase {
    ...
    func testPlaybackSongSyntheticAdapterAppliesSupportedVolumeColumnAndStillIgnoresEffectColumn() throws {
        ...
        let volumeAndEffectSong = makePlaybackSong(
            orderPatternIndices: [2],
            patternRowsByIndex: [
                2: [makePlaybackRow(index: 0, note: 49, instrument: 1, volumeColumn: 0x20, effectType: 0x04, effectParam: 0x43)]
            ],
            instrumentsByIndex: [1: PlaybackInstrument(index: 1, samples: [sample])],
            initialTiming: PlaybackTiming(speed: 4, bpm: 125)
        )
    ...
    func testPlaybackSongAdapterTonePortamentoVolumeSlideAndVolumeColumnTonePortamentoRemainDeferred() throws {
        let song = makePlaybackSong(
            orderPatternIndices: [2],
            patternRowsByIndex: [2: [
                makePlaybackRow(index: 0, note: 49, instrument: 1, volumeColumn: 0xF4, effectType: 0x05, effectParam: 0x20),
            ]],
            instrumentsByIndex: [1: PlaybackInstrument(index: 1, samples: [makeRampPlaybackSample(frameCount: 600, baseSampleRate: 100)])]
        )
    ...
    func testPlaybackSongAdapterOtherEffectColumnsRemainDeferredWith9xxDecoder() throws {
        ...
        let otherEffectSong = makePlaybackSong(
            orderPatternIndices: [2],
            patternRowsByIndex: [2: [makePlaybackRow(index: 0, note: 49, instrument: 1, effectType: 0x04, effectParam: 0x42)]],
            instrumentsByIndex: [1: PlaybackInstrument(index: 1, samples: [sample])]
        )
        let renderer = PlaybackSongOfflineRenderer()
        let config = MixerRenderConfig(sampleRate: 100, channelCount: 1)
        ...
        XCTAssertEqual(other.diagnostics.deferredCellFields.first?.effectType, 0x04)
    }
    ...
    func testRuntimeCMixerTraceDistinguishesChannelStopFromAllVoiceClear() {
        ...
        let stopChannel = traceWriter.events.first { $0.runtimeAction == "c_mixer_stop_channel" }
        XCTAssertEqual(stopChannel?.targetScope, "channel")
        XCTAssertEqual(stopChannel?.targetedAllVoices, false)
        XCTAssertEqual(stopChannel?.orderIndex, 0)
        XCTAssertEqual(stopChannel?.rowIndex, 0)
        XCTAssertEqual(stopChannel?.channelIndex, 0)
        XCTAssertEqual(stopChannel?.reason, "channel_stop")
