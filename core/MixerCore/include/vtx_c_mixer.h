...
// Pre-sanitized variant for callers that guarantee every sample frame is finite.
// The mixer still mallocs and owns a full payload copy, but uses memcpy instead of
// repeating the defensive per-frame finite check above.
...
// Scheduled pre-sanitized variant with the same ownership and finite-input
// contract as vtx_c_mixer_add_sample_voice_with_step_at_source_frame_pre_sanitized.
