...
envelope, panning, and effect decisions. Row/tick scheduling support is currently synthetic only and does not connect to real XM pattern, order, row, or instrument data. Generated WAV files, playback traces, comparison reports, and local modules must remain outside the repository.
