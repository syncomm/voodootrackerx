...
Focused Instrument Editor computer and graphical presses create the same request at the exact note pitch. Instrument Editor and tracker pattern preview resolve the selected instrument's exact 96-entry XM map without changing selection. Malformed maps, invalid targets, and unplayable mapped samples fail without falling back to the selected or first sample.
...
Blank documents begin with selected `I01`/`S01` destinations but no payload, so they are initially preview-unavailable. Imported or generated editable samples become previewable through the same request/resolver path.

Loaded modules may be auditionable before they become editable. Loaded-module instrument-note audition resolves instrument/keymap data from the loaded playback model, but it must not mutate loaded pattern data. Sample Editor audition resolves its selected sample directly.

## Loaded-Module Sample Availability

Loaded-module preview availability resolves the editor note-audition request against the app-level `PlaybackSong` instrument/sample model. The shared resolver validates the represented instrument, canonical XM note, exact keymap, mapped zero-based sample identity, and playable payload. A resolved sample reports a descriptor containing its instrument/sample indices, frame count, sample-header panning byte, sanitized forward or ping-pong loop metadata, source context, and copied PCM.

Tracker computer-key preview and focused Instrument Editor computer/graphical preview use the selected instrument's XM note map for the requested pitch. Main popups and Instrument Editor rows update canonical UI selection without document mutation or undo; selected sample is editing focus only and a change cannot redirect either note-driven route. Selection changes still cancel stale preview before switching context.

Only direct Sample Editor requests resolve the selected 1-based sample slot to its stored zero-based sample. Instrument-note requests resolve through the keymap. Either mode returns preview-unavailable for a missing, empty, or otherwise unplayable target and never silently falls back to `S01` or the first playable sample.

This resolver does not play audio, schedule voices, call CoreAudio, or change playback transport behavior. Loaded modules may be previewable before they become editable, and editable documents become previewable once they own real sample payload.
...
- Non-Edit-mode note-key `keyDown` may audition the selected loaded-module instrument through its note map when availability resolves to a previewable payload.
- Blank documents without real sample payload remain preview-unavailable.
...
- Instrument Editor audition reuses the current octave/instrument and availability/preview pipeline,
...
The first routing spike added `EditorNoteAuditionPreviewer`, which consumes an `EditorNoteAuditionRequest` plus an `EditorNoteAuditionAvailability` result. It attempts preview only for note-on requests whose loaded or editable descriptor reports real sample payload with a positive frame count and copied PCM. The previewer uses an injected sink so tests can verify request routing and metadata without CoreAudio or audio hardware.
...
- loaded-module or editable-document note-on requests
...
- the instrument/keymap target (or Sample Editor's direct target) must resolve to `.potentiallyAvailable(...)`
...
Preview is not attempted for documents without real sample payload, key-off requests, clear/delete input, navigation, unsupported keys, unavailable instrument/keymap/direct-sample targets, or samples without previewable PCM. Loaded modules remain read-only for pattern mutation, even when their sample descriptors are potentially previewable. Play/Stop transport, transport state, and normal song playback remain unrelated to editor preview and unchanged by this preview sink.
