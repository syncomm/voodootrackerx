...
Amiga-table path and use a neutral step fallback. The bounded adapter handles a
small set of regular effect-column cases. Minimal `Fxx` timing support treats
...
use the timing that was active at row start. `Cxx` set volume and `8xx` set
panning update the bounded adapter channel state and can update the currently
tracked active C voice on empty-note rows. `Axy` volume slide is a deterministic
row-level approximation for nonzero parameters; `A00` effect memory is not
implemented. `Hxy` global volume slide is also a minimal row-level bounded
offline approximation: the adapter keeps a clamped XM global volume value
`0...64`, defaults it to `64`, applies `Hxy` up/down changes once at the source
row start, multiplies active and later event gains by `globalVolume / 64`, and
...
voice on that channel. Subsequent ticks slide the active voice's sample step
toward that target by the `3xx` speed in XM linear-period units, clamping at the
target without overshoot. `3x0` uses the channel's remembered 3xx speed when
one exists. A `3xx` cell without a note continues toward the existing target;
if no target exists it is diagnosed as `no_target`. If no active adapted voice
exists it is diagnosed as `no_active_voice` and no playback is invented. This
first pass uses the existing linear-frequency calculation and the active
sample's relative note, finetune, base sample rate, and output sample rate; the
Amiga-table path remains deferred. Windowed renders carry the current 3xx target
sample-step state and integrate prior step changes into continuation source
positions where practical.
Minimal `1xx` portamento up and `2xx` portamento down support also applies only
in bounded/offline adapter renders. When a nonzero `1xx` or `2xx` cell has a
tracked active adapted voice, later ticks in the current row schedule generic C
mixer sample-step updates on that voice. This first pass uses the same XM
linear-period representation as `3xx`: `1xx` subtracts the parameter from the
current linear period per tick, `2xx` adds it per tick, and the adapter converts
each clamped period back into the active sample step with the active sample's
relative note, finetune, base sample rate, and output sample rate. Zero
parameters are diagnosed as effect-memory-deferred no-ops. No-active-voice and
non-linear/Amiga-table cases are diagnosed and do not invent playback. This is
a conservative deterministic approximation, not a claim of FT2/OpenMPT
tick-exact scaling parity. Windowed renders fold prior `1xx`/`2xx` step updates
into carried continuation voices and schedule in-window updates where practical.
Minimal nonzero `9xx` sample offset
...
skipped, producing deterministic silence for that trigger. Minimal `E9x`
retrigger support applies only in bounded/offline adapter renders. The adapter
decodes extended effect `E` subcommand `9`, computes retrigger ticks `x`, `2x`,
`3x`, and so on while the tick remains inside the current row speed, and
schedules fresh same-channel starts at the row's effective speed/BPM absolute
frames. Generated retrigger events reuse the tracked active event's selected
sample, source offset, pitch step, loop metadata, volume-envelope and panning
envelope mapping, and the channel's current volume/pan state. The first-pass
envelope policy is to restart the mapped envelope for each generated event.
The previously tracked same-channel voice is hard-cut at each retrigger frame
for deterministic replacement. `E90` is diagnosed as an ignored/deferred no-op;
...
Diagnostics identify remaining deferred pitch-modulation commands (`0xy`,
`4xy`, `5xy`, `6xy`, and `7xy`) with source coordinates and friendly labels so
local comparison reports can choose a narrow follow-up. Applied `1xx`/`2xx`
portamento slides and applied `3xx` tone portamento are reported separately with
current/target sample-step details where applicable.
...
source row, events emitted for that row use the post-command state, and
empty-note rows can schedule a generic frame-stamped gain/pan update for the
currently tracked active C voice. The C-backed offline mixer now smooths those
generic gain/pan update events with a fixed 32-frame deterministic micro-ramp.
...
Frame-stamped volume/panning updates that occurred before a boundary are folded
into continuation gain/pan, active gain/pan micro-ramp state is carried when a
boundary lands inside the fixed 32-frame ramp, prior `1xx`/`2xx`/`3xx`
sample-step updates are folded into continuation pitch state and source
position, and updates inside the new window are scheduled on the fresh C mixer
at local window frames.
...
pattern traversal effects, and full FT2/OpenMPT voice rules remain outside the
adapter scope. Minimal `E9x` retrigger, `ECx` note cut, and `EDx` note delay
are the only extended-effect voice timing exceptions currently applied by this
bounded offline adapter. Rows and note events contained wholly within a window
retain the existing bounded adapter behavior.
...
- Use `PlaybackSample.volume` as base event gain, multiplying it by the current
  ...
  `0...64`. Empty-note state commands also schedule a future gain update for
  the currently tracked active voice when one exists.
...
- Use a neutral pan default, or update Swift-side adapter channel panning from
  ...
  Empty-note panning state commands also schedule a future pan update for the
  currently tracked active voice when one exists.
...
- implement effect-column commands other than minimal `Fxx` speed/BPM timing,
  ...
  a row-level first-pass `Axy` volume slide, minimal `1xx`/`2xx` portamento
  up/down, minimal `3xx` tone portamento, minimal `E9x` retrigger, minimal
  `ECx` note cut, and minimal `EDx` note delay
...
| Active adapted voice sample step | C mixer voice sample-step update event | Minimal `1xx`/`2xx` portamento slides and minimal `3xx` tone portamento schedule generic absolute-frame sample-step updates for the active C voice. The C mixer has no XM effect knowledge; it only applies the adapter-computed source-sample step at the requested frame. Windowed renders fold prior completed step updates into the carried continuation step and integrate the previous step segments to compute continuation source position. | Full pitch modulation, Amiga-table period behavior, glissando, vibrato, arpeggio, and tick-exact FT2/OpenMPT semantics later. |
...
| Full volume-column and effect semantics may be mistaken as supported. | Apply only set-volume, set-panning, volume slides, fine volume slides, panning slides, `Cxx`, `8xx`, nonzero row-level `Axy`, minimal `1xx`/`2xx`, minimal `3xx`, and minimal row-level `Hxy` in the bounded adapter. Keep slides and `3xx` as first-pass approximations, defer vibrato/arpeggio/tremolo/`5xy`/volume-column tone portamento/`Gxx`/other effect behavior in diagnostics except the explicitly documented `Fxx`, nonzero `9xx`, `1xx`, `2xx`, `3xx`, `E9x`, `ECx`, `EDx`, `Cxx`, `8xx`, `Axy`, and `Hxy` cases, and document compatibility limits in test names. |
| Timing, sample-offset, retrigger, portamento, or Hxy support is mistaken for full effect parity. | Apply only minimal `Fxx` speed/BPM timing changes to following bounded rows, minimal nonzero `9xx` source starts on same-cell note triggers, minimal `1xx`/`2xx` linear-frequency portamento slides on the tracked active voice, minimal `3xx` linear-frequency tone portamento on the tracked active voice, minimal `E9x` same-channel retriggers from the tracked active voice, the documented volume/panning state commands, and row-level `Hxy` global-volume changes. Diagnose `F00`, `900`, zero-parameter `1xx`/`2xx`, `E90`, `H00`, both-nibble `Hxy` policy, out-of-range offsets, no-active `1xx`/`2xx`, no-active/no-target/no-speed `3xx`, no-active retriggers, out-of-row retriggers/cuts/delays, and all other effect-column commands without adding broad effect state. |
...
- Assert supported set-volume, set-panning, volume slide, fine volume slide, and
  ...
  diagnostics.
- Assert empty-note volume-column set-volume and set-panning update channel
  state and the currently tracked active voice from the update frame forward.
- Assert `Cxx` set volume and `8xx` set panning update bounded channel state
  and active voice gain/pan from the update frame forward.
- Assert nonzero `Axy` is diagnosed and applied as a row-level volume slide
  approximation.
- Assert `Hxy` global volume slide down/up affects active and later voices,
  clamps safely, diagnoses `H00` as no-op, diagnoses both-nibble up precedence,
  appears in JSON/correlation diagnostics, and carries through windowed renders.
- Assert subsequent note triggers use prior bounded channel volume/pan state,
  and windowed carryover preserves gain/pan updates across row-window
  boundaries.
- Assert supported active-voice gain/pan updates ramp over the fixed 32-frame
  offline C mixer micro-ramp, interrupted ramps restart from the current
  interpolated value, and `ECx` note cuts still use immediate hard-cut behavior.
...
- Assert `E9x` generates retrigger starts at ticks `x`, `2x`, `3x`, and so on
  while inside the row's effective speed, uses effective speed/BPM absolute
  frames, and hard-replaces the previous same-channel adapted voice
  deterministically.
- Assert `E9x` preserves selected sample, nonzero `9xx` source offset where
  already active, pitch step, gain, pan, loop metadata, and the documented
  fresh-event envelope restart policy.
- Assert `E90`, no-active-voice retriggers, and `x >= speed` retriggers are
  diagnosed without inventing samples or scheduling extra starts.
- Assert windowed renders apply in-window `E9x` retriggers for carried active
  voices where the bounded adapter can determine that state.
...
19. Done: window state carryover refinement for explicit row-windowed offline
    ...
    unchanged.
20. Done: minimal volume/panning state effects for bounded offline renders,
    applying empty-note volume-column set-volume/set-panning, `Cxx`, `8xx`,
    and nonzero row-level `Axy` to channel/active voice state before the later
    targeted `Hxy` global-volume pass.
...
23. Done: fixed 32-frame gain/pan update micro-ramping for bounded/offline
   C-backed renders, with diagnostics and windowed carryover of active ramp
   state.
24. Done: minimal `E9x` retrigger support for bounded/offline renders, with
    diagnostics, `E90` no-effect-memory reporting, same-channel replacement,
    metadata preservation, and windowed carryover handling where practical.
...
27. Done: minimal `1xx`/`2xx` portamento up/down support for bounded/offline
    renders, with deterministic linear-period sample-step slides, zero-parameter
    effect-memory deferral diagnostics, no-active diagnostics, clamping, and
...
Panning envelopes remain deferred. Minimal effect-column note cut (`ECx`) and
note delay (`EDx`) and minimal retrigger (`E9x`) are supported in bounded
offline renders only. `ECx` is a hard gain-zero cut at tick `x`, not
note-off/key-off envelope release; `EDx` delays only a normal same-cell note
trigger to tick `x`. `E9x` schedules fresh same-channel starts at ticks `x`,
...
- implement XM effect-column commands other than minimal `Fxx` speed/BPM timing,
  minimal nonzero `9xx` sample offset, `Cxx` set volume, `8xx` set panning,
  nonzero row-level `Axy` volume slide, minimal `E9x` retrigger, minimal `ECx`
  note cut, minimal `EDx` note delay, minimal `1xx`/`2xx` portamento up/down,
  minimal `3xx` tone portamento, and minimal row-level `Hxy` global volume slide
- implement `5xy` tone portamento plus volume slide or volume-column tone
  portamento
