...
Diagnostics identify remaining deferred pitch-modulation commands (`5xy`,
`7xy`, and deferred volume-column pitch commands) with source coordinates and
friendly labels so local comparison reports can choose a narrow follow-up.
Applied `0xy` arpeggio, applied `1xx`/`2xx` portamento slides, applied `3xx`
tone portamento, and applied `4xy`/`6xy` vibrato-family effects are reported
separately with sample-step details where applicable.
...
| `PlaybackSample.panning` | `SyntheticTrackerEvent.pan` | Each actual sample trigger initializes pan from the resolved keymap sample. Map byte `0` to `-1`, `128` exactly to `0`, and `255` to `+1` with monotonic piecewise interpolation; then apply supported same-row volume-column panning before `8xx`. | Panning envelopes and broader FT2/OpenMPT interaction parity later. |
...
| Risk | Mitigation |
| --- | --- |
| Scope expands into full XM playback. | The bounded adapter supports initial timing plus minimal `Fxx`, minimal nonzero `9xx`, minimal `E9x`, bounded orders, note triggers, sample-map/keymap selection with fallback, gain, sample-header initial pan, row-level volume/panning state, and loops only. Everything else is documented as deferred. |
...
- Assert channel volume carries while each actual sample trigger resets pan from
  its resolved sample before same-row panning commands; windowed carryover
  preserves gain/pan updates across row-window boundaries.
