...
| `scripts/stem-scaling-diagnostics.py` | Diagnostic / local-only; hyper-specific; candidate archive; candidate CLI subcommand | Sums local reference stems and optionally compares the sum against a full render. | Archived audio history, `tools/audio_compare_tests.py`; imports `audio-compare.py`. | Handles local stem WAVs that may be derived from private modules. | Yes. | Keep for now. | Archive or fold into `audio_compare stems` after test coverage is moved to the unified CLI. |
...
| `scripts/summarize-xm-effect-coverage.py` | Active workflow; diagnostic / local-only; candidate CLI subcommand | Summarizes XM effect coverage from bounded offline diagnostics or runtime traces. | `docs/audio-comparison.md`, `docs/playback-trace.md`, archived reports, `BoundedXMRenderTool.swift` help text, `tools/audio_compare_tests.py`. | Reads diagnostics/traces that may come from private modules; output should use redacted labels. | Yes. | Keep for now. | Fold into `effect_coverage`; keep compatibility path for existing docs. |
| `scripts/summarize-xm-residual-effect-scan.py` | Active local planning helper; diagnostic / local-only; candidate CLI subcommand | Scans a private XM corpus label map for residual effect-memory and volume-column gaps. | `tools/xm_residual_effect_scan_tests.py`; archived reports. | Yes; reads a local label map with private paths and emits public-safe labels/counts. | Yes; default label map and outputs are under `/tmp`. | Keep for now. | Fold into `residual_scan`; keep strict redaction tests before any move. |
...
| --- | --- | --- |
| `scripts/stem-scaling-diagnostics.py` | It is a narrow stem reconstruction helper and current references are test coverage plus archived history. | Confirm no current docs/prompts use it, migrate or retire tests, then add a compatibility wrapper or archive note. |
