...
| VTX-G-003 | Promoted from scheduled audit debt to a release blocker by final maintainer evidence; remediated, validated, and merged before the final `GO` gate | `fix/document-replacement-discard-confirmation`, PR #403, `174639c` | New and Open require an explicit native warning before replacing meaningful editable song content. Eligibility is a semantic comparison with canonical File-New content, not a general dirty or Save lifecycle; navigation and selection alone remain pristine, and exporting does not suppress protection. Cancel, picker Cancel, load failure, and stale confirmation preserve the exact editable document, identity, revision, and undo history. Pristine editable documents and loaded read-only sources retain their existing low-friction behavior, loaded source files remain untouched, and Save/Save As remain disabled. |
