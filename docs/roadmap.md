...
- Scope: separate follow-up if output-host A/B diagnostics confirm that runtime rendering continues after the planned adapter event stream ends, especially when final sustained or looped voices continue indefinitely. Define the intended runtime stop/silence/tail rule independently from capture duration.
