...
- Classification: the remaining mismatch is not full-mix accumulation-only. The strongest evidence points to per-channel/per-sample level or PCM-scaling parity for the dominant instrument/sample, with smaller residual timbre differences after gain normalization. Timing, panning/stereo placement, loop-boundary crossing, sample-step updates, replacement ramps, and SINC8/default reference mode remain weakened as primary causes for the focused windows.
...
- Status: diagnostics-first with normal playback unchanged. Recommended next PR: Sample PCM Decode / Scaling Parity Fix or a dominant-sample steady-state microfixture comparing the same PCM, sample step, loop settings, and gain against the isolated-stem evidence.
