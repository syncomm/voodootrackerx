...
- Runtime C mixer stabilization diagnostics now include a local trace summary helper for post-ramping A/B passes. The helper summarizes runtime output health counters, stop/replacement paths, hard-stop evidence, normal-playback clear-all counts, active/loaded voice ranges, update categories, event bursts, and whether runtime traces show the same high-level update/effect categories as the bounded offline adapter path. Recent local listening still reports hard cuts/stumbles in the opt-in runtime C mixer while default AVAudio playback and offline C-backed WAV renders are cleaner, so the next investigation is likely runtime event/state scheduling parity or an offline-adapter event-stream bridge rather than C mixer DSP, headroom, parser, or tracker UI work.
