...
- Scope: future bridge that can make tracker follow observe already-computed runtime C mixer sample-time position state when the experimental backend is selected, without changing static-highlight viewport behavior or making the C mixer default.
- Verification: focused UI/model tests plus manual visual verification for anchor row, gutter alignment, wraparound, and no phantom rows; default AVAudio backend behavior must remain unchanged.
