...
- The developer-only bounded XM render helper keeps its conservative 60-second default clamp while allowing explicit longer local candidate renders through documented `--seconds` / `--max-frames` controls gated by `--allow-long-render`. A later focused PR should add calculated bounded song-end duration and short tail handling without changing these hard debug caps or enabling default looping.
