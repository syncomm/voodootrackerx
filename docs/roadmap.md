...
- Runtime C mixer AVAudio output-device and render-callback isolation diagnostics now report main-mixer input/output format, output-node latency/presentation latency, hashed output-device identity, hardware IO/latency/safety-offset fields, engine configuration-change counts, graph/route change counts, callback thread id/main-thread status, event-queue producer/consumer thread ids, diagnostic callback allocation-risk evidence, and a local-only follow-publication disable flag for UI/callback isolation. Output tap capture and any actual AVAudio graph or hardware-route fix remain separate follow-up work. AVAudio remains the default backend, the C mixer remains opt-in only, offline rendering is unchanged, tracker viewport code is unchanged, and parser architecture is unchanged.
