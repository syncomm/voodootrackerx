...
- Runtime C mixer stabilization diagnostics now include a local trace summary helper for post-ramping A/B passes. The helper summarizes runtime output health counters, stop/replacement paths, hard-stop evidence, normal-playback clear-all counts, active/loaded voice ranges, update categories, event bursts, and whether runtime traces show the same high-level update/effect categories as the bounded offline adapter path. The experimental runtime C mixer now precomputes a runtime adapter event plan from the bounded/offline `PlaybackSong` adapter and consumes supported plan events when available, with trace fields for event source, plan generation, planned/consumed/skipped counts, adapter categories, row/order mapping, and fallback-to-simple-runtime reasons. AVAudio remains the default backend, the C mixer remains opt-in only, unsupported XM effects remain unsupported, and sample-time tracker-follow alignment plus broader runtime stabilization remain separate future work.
