...
- Runtime C mixer event/update bridging now applies supported gain/pan/sample-step control updates to the current channel-tagged C mixer voice instead of treating them as a single deferred no-op class. Applied update trace rows distinguish gain/pan, sample-step, and combined updates; missing target data and unsupported update values remain explicit deferred diagnostics. The backend remains experimental and opt-in, and this does not add new XM effects, change tracker-follow behavior, or switch the default backend.
