...
### PR 2.7.11l — Runtime C Mixer Playback Follow / Sample-Time Position Bridge
- Scope: expose diagnostics that map the runtime C mixer rendered frame cursor back to the planned adapter order/pattern/row/tick timeline and compare that position with `PlaybackEngine` order/pattern/row/tick at the same trace point. Keep this diagnostics/future-bridge only; do not change tracker viewport rendering, Stop behavior, keyboard shortcuts, runtime event timing, C mixer DSP semantics, parser architecture, offline render behavior, or the default AVAudio backend.
- Verification: focused synthetic resolver and runtime trace tests, synthetic trace summary tests for row-transition deltas and PlaybackEngine-vs-C-mixer mismatches, existing runtime/offline render tests, app build/test, local-only runtime traces under ignored paths, and no private/local module fixtures.
- Status: done.
