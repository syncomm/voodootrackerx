...
final class VTXRenderBoundedXMTests: XCTestCase {
    ...
    func testRendersDiagnosticsJSONWhenRequested() throws {
        ...
        let gainConstruction = try XCTUnwrap(firstEvent["gain_construction"] as? [String: Any])
        XCTAssertEqual(gainConstruction["sample_volume_raw_estimate"] as? Int, 64)
    ...
    func testDiagnosticsJSONIncludesEAxAndEBxFineVolumeSlides() throws {
        ...
        // Each explicit trigger loads default 64 before its fine volume slide.
        XCTAssertEqual(sameCellEAx["effective_volume_before"] as? Int, 64)
        XCTAssertEqual(sameCellEAx["effective_volume_after"] as? Int, 64)
        XCTAssertEqual(sameCellEBx["active_voice_updated"] as? Bool, false)
        XCTAssertEqual(sameCellEBx["effective_volume_before"] as? Int, 64)
        XCTAssertEqual(sameCellEBx["effective_volume_after"] as? Int, 62)
        XCTAssertEqual(sameCellEAxEvent["gain"] as? Double, 1)
        XCTAssertEqual(sameCellEBxEvent["gain"] as? Double, 62.0 / 64.0)
    ...
    func testDiagnosticsJSONIncludes6xyVibratoVolumeSlide() throws {
        ...
        let zero = try XCTUnwrap(sixxyUpdates.first { $0["effect_param"] as? Int == 0 })
        ...
        let noVoice6xy = try XCTUnwrap(sixxyEffects.first {
            $0["current_status"] as? String == "no_active_voice"
        })
        ...
        XCTAssertEqual(render["vibrato_volume_slide_6xy_zero_speed_or_depth_effect_memory_deferred_count"] as? Int, 0)
        XCTAssertEqual(render["vibrato_volume_slide_6xy_deferred_count"] as? Int, 0)
        XCTAssertEqual(render["vibrato_volume_slide_6xy_memory_applied_count"] as? Int, 4)
        XCTAssertEqual(render["vibrato_volume_slide_6xy_memory_missing_count"] as? Int, 0)
        XCTAssertEqual(render["vibrato_volume_slide_6xy_scheduled_sample_step_update_count"] as? Int, 12)
        XCTAssertEqual(render["vibrato_volume_slide_6xy_scheduled_gain_update_count"] as? Int, 9)
        XCTAssertEqual(split.block.interleavedPCM, result.block.interleavedPCM)
        XCTAssertEqual(summary["vibrato_volume_slide_6xy_volume_slide_applied"] as? Int, 15)
        XCTAssertEqual(summary["vibrato_volume_slide_6xy_no_active_voice"] as? Int, 3)
        XCTAssertEqual(summary["vibrato_volume_slide_6xy_zero_param_effect_memory_deferred"] as? Int, 0)
        XCTAssertEqual(summary["vibrato_volume_slide_6xy_scheduled_gain_update_count"] as? Int, 9)
        XCTAssertEqual(events.count, 2)
        XCTAssertEqual(sixxyUpdates.count, 15)
        ...
        XCTAssertEqual(activeDown["effective_volume_before"] as? Int, 35)
        XCTAssertEqual(activeDown["effective_volume_after"] as? Int, 20)
        XCTAssertEqual(noActive["active_voice_updated"] as? Bool, false)
        XCTAssertEqual(noActive["no_active_voice"] as? Bool, true)
        XCTAssertEqual(zero["status"] as? String, "applied")
        XCTAssertEqual(zero["effect_memory_deferred"] as? Bool, false)
        XCTAssertEqual(zero["effect_memory_reused"] as? Bool, true)
        XCTAssertEqual(zero["memory_source_effect_param"] as? Int, 15)
        XCTAssertEqual(zero["effective_volume_after"] as? Int, 0)
        XCTAssertEqual(sameCell["active_voice_updated"] as? Bool, true)
        // The new note reloads default 64; 6xy still starts sliding at tick 1.
        XCTAssertEqual(sameCell["effective_volume_before"] as? Int, 64)
        XCTAssertEqual(sameCell["effective_volume_after"] as? Int, 62)
        XCTAssertEqual(sameCellEvent["gain"] as? Double, 1)
        ...
        XCTAssertEqual(applied6xy["scheduled_sample_step_update_count"] as? Int, 3)
        ...
        XCTAssertEqual(memory600["scheduled_sample_step_update_count"] as? Int, 3)
        XCTAssertEqual(noVoice6xy["ignored_as_no_op"] as? Bool, true)
        XCTAssertEqual(noVoice6xy["effect_memory_missing"] as? Bool, false)
        XCTAssertTrue(noVoice6xy["memory_unavailable_reason"] is NSNull)
        XCTAssertTrue(effects.contains { $0["effect_label"] as? String == "6xy vibrato + volume slide" && $0["status"] as? String == "applied" })
        XCTAssertFalse(effects.contains { $0["effect_label"] as? String == "6xy vibrato + volume slide" && $0["status"] as? String == "ignored/no-op" })
    }
    ...
    func testDiagnosticsJSONIncludesE4xVibratoControl() throws {
        ...
        let suppressed = try XCTUnwrap(controls.first { $0["retrigger_suppressed"] as? Bool == true })
        ...
        XCTAssertEqual(render["e4x_vibrato_control_stored_count"] as? Int, 2)
        XCTAssertEqual(render["e4x_vibrato_control_applied_count"] as? Int, 2)
        XCTAssertEqual(render["e4x_vibrato_control_unsupported_waveform_count"] as? Int, 0)
        XCTAssertEqual(render["e4x_vibrato_control_deferred_count"] as? Int, 0)
        ...
        XCTAssertEqual(suppressed["unsupported_waveform"] as? Bool, false)
        XCTAssertEqual(suppressed["retrigger_suppressed"] as? Bool, true)
    ...
    func testWindowedRenderMatchesFor6xyVibratoVolumeSlideWithoutRetrigger() throws {
        ...
        XCTAssertEqual(render["vibrato_volume_slide_6xy_scheduled_gain_update_count"] as? Int, 9)
        XCTAssertEqual(render["vibrato_volume_slide_6xy_scheduled_sample_step_update_count"] as? Int, 9)
        XCTAssertEqual(split.block.interleavedPCM, result.block.interleavedPCM)
        XCTAssertEqual(windowed.block.interleavedPCM, result.block.interleavedPCM)
    }
    ...
    func testDiagnosticsJSONCountsDeferredPitchModulationEffectsWithCoordinates() throws {
        ...
        let tonePortamentoEffects = try XCTUnwrap(object["tone_portamento_effects"] as? [[String: Any]])
        let volumeTonePortamento = try XCTUnwrap(tonePortamentoEffects.first { $0["effect_label"] as? String == "volume-column tone portamento" })
        let vibratoEffects = try XCTUnwrap(object["vibrato_effects"] as? [[String: Any]])
        let fourxyEffects = vibratoEffects.filter { $0["effect_type"] as? Int == 0x04 }
        let fivexyEffects = try XCTUnwrap(object["tone_portamento_volume_slide_5xy_effects"] as? [[String: Any]])
        let sixxyEffects = try XCTUnwrap(object["vibrato_volume_slide_6xy_effects"] as? [[String: Any]])
        ...
        XCTAssertEqual(render["vibrato_4xy_effect_count"] as? Int, 1)
        XCTAssertEqual(render["vibrato_4xy_applied_count"] as? Int, 1)
        XCTAssertEqual(render["vibrato_4xy_scheduled_sample_step_update_count"] as? Int, 5)
        XCTAssertEqual(render["vibrato_volume_slide_6xy_effect_count"] as? Int, 1)
        XCTAssertEqual(render["vibrato_volume_slide_6xy_applied_count"] as? Int, 1)
        XCTAssertEqual(render["vibrato_volume_slide_6xy_zero_speed_or_depth_effect_memory_deferred_count"] as? Int, 0)
        ...
        XCTAssertEqual(render["volume_column_tone_portamento_effect_count"] as? Int, 1)
        XCTAssertEqual(render["volume_column_tone_portamento_applied_count"] as? Int, 0)
        XCTAssertEqual(render["volume_column_tone_portamento_no_active_voice_count"] as? Int, 1)
        XCTAssertEqual(render["volume_column_tone_portamento_scheduled_sample_step_update_count"] as? Int, 0)
        XCTAssertEqual(render["tone_portamento_volume_slide_5xy_effect_count"] as? Int, 1)
        XCTAssertEqual(render["tone_portamento_volume_slide_5xy_applied_count"] as? Int, 0)
        XCTAssertEqual(render["tone_portamento_volume_slide_5xy_no_active_voice_count"] as? Int, 1)
        XCTAssertEqual(render["tone_portamento_volume_slide_5xy_scheduled_sample_step_update_count"] as? Int, 0)
        ...
        XCTAssertEqual(fourxyEffects.first?["scheduled_sample_step_update_count"] as? Int, 5)
        XCTAssertEqual(fivexyEffects.count, 1)
        XCTAssertEqual(fivexyEffects.first?["effect_label"] as? String, "5xy tone portamento + volume slide")
        XCTAssertEqual(fivexyEffects.first?["current_status"] as? String, "no_active_voice")
        XCTAssertEqual(fivexyEffects.first?["active_voice_found"] as? Bool, false)
        ...
        XCTAssertFalse(coordinates.contains { $0["effect_label"] as? String == "volume-column tone portamento" })
    ...
    func testDiagnosticsJSONReportsFinePortamentoDownE2xDetails() throws {
        ...
        let rows = [
            PlaybackRow(index: 0, cells: [
                PlaybackCell(note: 49, instrument: 1, volumeColumn: 0, effectType: 0, effectParam: 0),
                PlaybackCell(note: 0, instrument: 0, volumeColumn: 0, effectType: 0x0E, effectParam: 0x2F),
            ]),
            PlaybackRow(index: 1, cells: [
                PlaybackCell(note: 0, instrument: 0, volumeColumn: 0, effectType: 0x0E, effectParam: 0x21),
            ]),
            PlaybackRow(index: 2, cells: [
                PlaybackCell(note: 0, instrument: 0, volumeColumn: 0, effectType: 0x0E, effectParam: 0x20),
            ]),
        ]
        let song = PlaybackSong(
            title: "fine-portamento-down-diagnostics",
            orders: [PlaybackOrderEntry(orderIndex: 0, patternIndex: 2)],
            patternsByIndex: [2: PlaybackPattern(index: 2, rows: rows)],
            instrumentsByIndex: [1: PlaybackInstrument(index: 1, samples: [sample])],
            restartOrderIndex: 0,
        ...
        let object = PlaybackSongDiagnosticsJSONExporter.jsonObject(from: result)
        let render = try XCTUnwrap(object["render"] as? [String: Any])
        let diagnostics = try XCTUnwrap(object["fine_portamento_down_effects"] as? [[String: Any]])
        let aliasDiagnostics = try XCTUnwrap(object["e2x_fine_portamento_down_effects"] as? [[String: Any]])
        ...
        XCTAssertEqual(render["e2x_fine_portamento_down_effect_count"] as? Int, 3)
        XCTAssertEqual(render["e2x_fine_portamento_down_detected_count"] as? Int, 3)
        XCTAssertEqual(render["e2x_fine_portamento_down_applied_count"] as? Int, 1)
        XCTAssertEqual(render["e2x_fine_portamento_down_no_active_voice_count"] as? Int, 1)
        XCTAssertEqual(render["e2x_fine_portamento_down_zero_amount_effect_memory_deferred_count"] as? Int, 1)
        XCTAssertEqual(render["e2x_fine_portamento_down_deferred_count"] as? Int, 1)
        XCTAssertEqual(render["e2x_fine_portamento_down_scheduled_sample_step_update_count"] as? Int, 1)
        ...
        XCTAssertLessThan(stepAfter, stepBefore)
    ...
    func testDiagnosticsJSONIncludesEnvelopeSemanticsAndKeyOffFields() throws {
        ...
        XCTAssertEqual(volumeEnvelope["clock_policy"] as? String, "canonical_fxx_tick_plan")
        XCTAssertEqual(volumeEnvelope["point_mapping_policy"] as? String, "legacy_trigger_frame_projection")
        XCTAssertEqual(volumeEnvelope["position_domain"] as? String, "xm_ticks")
        XCTAssertEqual(volumeEnvelope["advance_policy"] as? String, "publish_tick_target_and_hold")
        XCTAssertEqual(volumeEnvelope["first_audible_frame_policy"] as? String, "note_start_uses_position_zero_before_advance")
        XCTAssertEqual(volumeEnvelope["sustain_policy"] as? String, "hold_logical_sustain_tick_until_release")
        XCTAssertEqual(volumeEnvelope["key_off_policy"] as? String, "release_before_rendering_key_off_frame")
        XCTAssertEqual(volumeEnvelope["release_policy"] as? String, "continue_from_current_or_sustain_position_after_key_off")
        XCTAssertEqual(volumeEnvelope["loop_policy"] as? String, "wrap_on_end_tick_except_released_sustain_end")
        XCTAssertEqual(volumeEnvelope["loop_end_policy"] as? String, "exclusive")
        XCTAssertEqual(volumeEnvelope["loop_after_key_off"] as? Bool, false)
        ...
        let points = try XCTUnwrap(volumeEnvelope["points"] as? [[String: Any]])
        XCTAssertEqual(points.count, 2)
        XCTAssertEqual(points.first?["position_frame"] as? Int, 0)
        XCTAssertEqual(points.first?["value"] as? Double, 1.0)
        ...
        XCTAssertEqual(volumeEnvelope["key_off_frame"] as? Int, 1)
        XCTAssertEqual(volumeEnvelope["fadeout_start_frame"] as? Int, 1)
        XCTAssertEqual(volumeEnvelope["fadeout_value"] as? Int, 65_536)
        XCTAssertEqual(volumeEnvelope["fadeout_applied"] as? Bool, true)
        XCTAssertTrue(volumeEnvelope["fadeout_frame_decrement"] is NSNull)
        XCTAssertEqual(volumeEnvelope["fadeout_tick_decrement"] as? Int, 65_536)
        XCTAssertEqual(volumeEnvelope["envelope_tick_frame_at_start"] as? Int, 0)
        XCTAssertTrue(volumeEnvelope["envelope_position_frame_at_start"] is NSNull)
        XCTAssertTrue(volumeEnvelope["envelope_position_frame_after_advance_at_start"] is NSNull)
        XCTAssertEqual(volumeEnvelope["envelope_value_at_start"] as? Double, 1.0)
        XCTAssertTrue(volumeEnvelope["envelope_segment_index_at_start"] is NSNull)
        XCTAssertEqual(volumeEnvelope["envelope_sustain_held_at_start"] as? Bool, false)
        XCTAssertTrue(volumeEnvelope["envelope_loop_active_at_start"] is NSNull)
        XCTAssertTrue(volumeEnvelope["envelope_loop_taken_count_at_start"] is NSNull)
        XCTAssertEqual(volumeEnvelope["key_on_at_start"] as? Bool, true)
        XCTAssertEqual(volumeEnvelope["fadeout_value_at_start"] as? Double, 1.0)
        XCTAssertEqual(volumeEnvelope["final_voice_gain_at_start"] as? Double, 1.0)
        XCTAssertEqual(volumeEnvelope["envelope_tick_frame_at_key_off"] as? Int, 1)
        XCTAssertTrue(volumeEnvelope["envelope_position_frame_at_key_off"] is NSNull)
        XCTAssertTrue(volumeEnvelope["envelope_position_frame_after_advance_at_key_off"] is NSNull)
        XCTAssertEqual(volumeEnvelope["envelope_value_at_key_off"] as? Double, 0.5)
        XCTAssertEqual(volumeEnvelope["key_on_at_key_off"] as? Bool, false)
        XCTAssertTrue(volumeEnvelope["envelope_loop_taken_count_at_key_off"] is NSNull)
        XCTAssertEqual(volumeEnvelope["fadeout_value_at_key_off"] as? Double, 0.0)
        XCTAssertEqual(volumeEnvelope["final_voice_gain_at_key_off"] as? Double, 0.0)
        let snapshots = try XCTUnwrap(volumeEnvelope["diagnostic_snapshots"] as? [[String: Any]])
        XCTAssertEqual(snapshots.map { $0["label"] as? String }, ["start", "key_off"])
        XCTAssertTrue(snapshots.first?["position_frame_after_advance"] is NSNull)
        XCTAssertEqual(snapshots.first?["position_tick"] as? Int, 0)
        XCTAssertTrue(snapshots.first?["loop_taken_count"] is NSNull)
        let targets = try XCTUnwrap(volumeEnvelope["semantic_targets"] as? [[String: Any]])
        XCTAssertEqual(targets.map { $0["event_index"] as? Int }, [0, 0, 0, 0])
        XCTAssertEqual(targets.map { $0["channel_index"] as? Int }, [0, 0, 0, 0])
        XCTAssertEqual(targets.map { $0["scheduled_frame"] as? Int }, [0, 1, 2, 3])
        XCTAssertEqual(targets.map { $0["fadeout_accumulator"] as? Int }, [32_768, 0, 0, 0])
