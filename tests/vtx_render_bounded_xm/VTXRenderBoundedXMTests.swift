...
final class VTXRenderBoundedXMTests: XCTestCase {
    ...
    func testDiagnosticsJSONIncludesEnvelopeSemanticsAndKeyOffFields() throws {
        ...
        XCTAssertEqual(volumeEnvelope["clock_policy"] as? String, "xm_ticks_mapped_to_output_frames")
        XCTAssertEqual(volumeEnvelope["point_mapping_policy"] as? String, "floor_xm_tick_times_frames_per_tick_at_event")
        XCTAssertEqual(volumeEnvelope["position_domain"] as? String, "output_frames")
        XCTAssertEqual(volumeEnvelope["advance_policy"] as? String, "evaluate_then_advance_one_output_frame")
        XCTAssertEqual(volumeEnvelope["first_audible_frame_policy"] as? String, "note_start_uses_position_zero_before_advance")
        XCTAssertEqual(volumeEnvelope["sustain_policy"] as? String, "hold_at_sustain_frame_while_key_on_before_loop")
        ...
        XCTAssertEqual(volumeEnvelope["loop_policy"] as? String, "inclusive_frame_loop_while_key_on_after_sustain_check")
        XCTAssertEqual(volumeEnvelope["loop_end_policy"] as? String, "inclusive")
        ...
        XCTAssertEqual(volumeEnvelope["fadeout_frame_decrement"] as? Double, 0.5)
        XCTAssertEqual(volumeEnvelope["envelope_tick_frame_at_start"] as? Int, 0)
        XCTAssertEqual(volumeEnvelope["envelope_position_frame_at_start"] as? Int, 0)
        XCTAssertEqual(volumeEnvelope["envelope_position_frame_after_advance_at_start"] as? Int, 1)
        XCTAssertEqual(volumeEnvelope["envelope_value_at_start"] as? Double, 1.0)
        XCTAssertEqual(volumeEnvelope["envelope_segment_index_at_start"] as? Int, 0)
        XCTAssertEqual(volumeEnvelope["envelope_sustain_held_at_start"] as? Bool, false)
        XCTAssertEqual(volumeEnvelope["envelope_loop_active_at_start"] as? Bool, false)
        XCTAssertEqual(volumeEnvelope["envelope_loop_taken_count_at_start"] as? Int, 0)
        ...
        XCTAssertEqual(volumeEnvelope["envelope_position_frame_at_key_off"] as? Int, 1)
        XCTAssertEqual(volumeEnvelope["envelope_position_frame_after_advance_at_key_off"] as? Int, 2)
        ...
        XCTAssertEqual(volumeEnvelope["envelope_loop_taken_count_at_key_off"] as? Int, 0)
        XCTAssertEqual(volumeEnvelope["fadeout_value_at_key_off"] as? Double, 1.0)
        XCTAssertEqual(volumeEnvelope["final_voice_gain_at_key_off"] as? Double, 0.5)
        ...
        XCTAssertEqual(snapshots.first?["position_frame_after_advance"] as? Int, 1)
        XCTAssertEqual(snapshots.first?["loop_taken_count"] as? Int, 0)
