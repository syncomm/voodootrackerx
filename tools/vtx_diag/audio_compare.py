#!/usr/bin/env python3
"""Compare two local WAV renders and run the local comparison smoke workflow."""
...
COMPARE_DESCRIPTION = "Compare a reference WAV render with a VoodooTracker X candidate WAV."


def add_arguments(parser: argparse.ArgumentParser) -> None:
    """Add the authoritative comparator arguments to ``parser``."""
...
def build_argument_parser() -> argparse.ArgumentParser:
    """Build the standalone compatibility parser for the comparator."""
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    """Parse standalone comparator arguments."""

    return build_argument_parser().parse_args(argv)


def run(args: argparse.Namespace) -> int:
    """Run one comparison from already-parsed arguments."""
