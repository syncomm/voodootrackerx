...
FLOAT_DIGITS = 9
DISCONTINUITIES_DESCRIPTION = "Analyze a local WAV for likely click/crackle discontinuity evidence."
...
def add_arguments(parser: argparse.ArgumentParser) -> None:
    """Add the authoritative discontinuity analysis arguments to ``parser``."""
...
def build_argument_parser() -> argparse.ArgumentParser:
    """Build the standalone compatibility parser for discontinuity analysis."""

    parser = argparse.ArgumentParser(description=DISCONTINUITIES_DESCRIPTION)
    add_arguments(parser)
    return parser


def parse_args(argv: list[str]) -> argparse.Namespace:
    """Parse standalone discontinuity analysis arguments."""

    return build_argument_parser().parse_args(argv)


def run(args: argparse.Namespace) -> int:
    """Run discontinuity analysis from parsed arguments."""
...
def main(argv: list[str]) -> int:
    """Run the standalone discontinuity analysis CLI."""
