"""Local-only bounded candidate/reference WAV smoke comparison workflow."""
...
def add_arguments(parser: argparse.ArgumentParser) -> None:
    """Add the authoritative local smoke arguments to ``parser``."""

    parser.add_argument(
        "--candidate",
        required=True,
        type=Path,
        help="Existing VTX candidate WAV path",
    )
...
def build_argument_parser() -> argparse.ArgumentParser:
    """Build the standalone compatibility parser for the smoke workflow."""

    parser = argparse.ArgumentParser(description=SMOKE_DESCRIPTION)
    add_arguments(parser)
    return parser


def parse_args(argv: list[str]) -> argparse.Namespace:
    """Parse standalone local smoke arguments."""
...
def run(args: argparse.Namespace) -> int:
    """Run one local comparison smoke from already-parsed arguments."""
    ...
    result = audio_compare.main(compare_arguments)
    if result != 0:
...
def main(argv: list[str]) -> int:
    """Run the standalone compatibility smoke command."""
